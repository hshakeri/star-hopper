...
class StarHopperGame {
  ...
  canNPCTrade(npc, signal = null) {
    ...
    if (npc.hiddenInCave || npc.returningFromCave || npc.rescuePending || npc.shelterReason || (npc.panicTimer || 0) > 0) return false;
    return true;
  }
  ...
  releaseVillagersFromCaves(options = {}) {
    ...
    for (const obj of this.interactiveObjects || []) {
      ...
      if (hadShelterState) {
        this.releaseNPCFromCave(obj, { returnHome: true, exitAtCave: !!obj.hiddenInCave });
        released++;
      } else {
  ...
  updateVillagerShelterStates() {
    ...
    for (const obj of this.interactiveObjects) {
      ...
      if (!obj.hiddenInCave && obj.rescuePending) {
        ...
        this.releaseNPCFromCave(obj, { returnHome: true, exitAtCave: true });
        touchNeedsSync = true;
  ...
  getLabChainRunCue() {
    ...
    const rewardLine = reward > 0 ? `+${reward} XP` : `x${nextCombo}`;
    const displayRemaining = Math.max(1, remaining);
    const contractLine = state === "paused"
      ? "FRESH TEST ONLY"
      : `${displayRemaining} FRESH TEST${displayRemaining === 1 ? "" : "S"} -> ${rewardLine}`;
  ...
  drawLabChainRunCue(ctx) {
    ...
    if (cue.contractLine) {
      ctx.fillStyle = cue.state === "paused" ? "#cbd5e1" : "#fde68a";
      ctx.font = "bold 7px 'Share Tech Mono', monospace";
      ctx.fillText(this.fitCardText(ctx, cue.contractLine, w - 18), x + 10, y + 53);
    }
