// navigator.js - Handles Spaceship Orbital Navigation simulation (Velocity Verlet)
...
function initNavigatorMode() {
  ...
  loadNavigatorMission(activeNavMissionIndex);
  renderNavigatorMissions();
}
...
function loadNavigatorMission(index) {
  ...
  clearNavigatorConsole();
  logNavigatorConsole(`Loaded Mission: ${mission.title}`, "success");
  logNavigatorConsole(`Starter command loaded. Click 'Enter' or type to execute.`, "info");
...
function renderNavigatorMissions() {
...
function logNavigatorConsole(msg, type = "info") {
...
function clearNavigatorConsole() {
  const history = document.getElementById("navigator-console-history");
  if (history) {
...
function runNavigatorCommands(commandString) {
  ...
  logNavigatorConsole(`Executing: ${commandString}`, "info");
  ...
  parts.forEach(part => {
    ...
    try {
      if (cmd.startsWith("point_at")) {
        ...
        logNavigatorConsole(`Syntax error: Unknown command '${cmd}'`, "error");
      }
    } catch (e) {
      logNavigatorConsole(`Syntax error in: ${cmd}`, "error");
    }
  });
...
function updateNavigator(game) {
  ...
  if (!currentAction && commandQueue.length > 0) {
    ...
    if (currentAction.type === 'rotate') {
      ...
      logNavigatorConsole(`Ship oriented towards ${currentAction.target}.`, "info");
      ...
      logNavigatorConsole(`Firing thrusters: power=${currentAction.power}, duration=${currentAction.duration}s`, "info");
    } else if (currentAction.type === 'wait') {
      actionTimeRemaining = currentAction.duration;
    } else if (currentAction.type === 'warp') {
      timeWarpFactor = currentAction.factor;
      logNavigatorConsole(`Time warp multiplier set to ${timeWarpFactor}x`, "info");
      currentAction = null;
    }
  }
  ...
  if (!orbitalMissionsCompleted.has(activeMission.id)) {
    if (activeMission.validate(ship)) {
      ...
      logNavigatorConsole(`Mission Success: ${activeMission.title}! Coordinates locked!`, "success");
      renderNavigatorMissions();
  ...
  if (distEarth < earth.r) {
    logNavigatorConsole("COLLISION ALERT: Ship crashed into Earth!", "error");
    loadNavigatorMission(activeNavMissionIndex);
  }
}

// Predict trajectory for dotted preview line
function getPredictedTrajectory(stepsCount = 150) {
  if (!ship) return [];
...
function drawNavigator(game) {
  ...
  const path = getPredictedTrajectory();
  ctx.beginPath();
...
// Hook Input Keyboard enter listener inside Navigator console
