...
function renderAttemptLog(game) {
  ...
  if (!box || !game) return;
  const rows = attemptLogRows(game.currentPlanetIndex);
  if (!rows.length) { box.innerHTML = '<div class="attempt-log-empty">No attempts yet — every run lands here as an experiment.</div>'; return; }
  box.innerHTML = "";
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = { AttemptLog, attemptLogStart, attemptLogCode, attemptLogFinish, attemptPredictionVerdict, predictNextAttempt };
}
